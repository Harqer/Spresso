...
class TestCallPromotionAgent:
    ...
    @pytest.mark.asyncio
    async def test_raises_on_agent_unavailable(self) -> None:
        """Raises RuntimeError when agent is unavailable (fail-fast)."""
        context: PromotionContextInput = {
            "product_id": "prod_1",
            "product_name": "Test Product",
            "base_price_cents": 2500,
        ...
        with pytest.raises(RuntimeError, match="Promotion agent returned no decision"):
            await call_promotion_agent(context, mock_client)
...
class TestGetPromotionForProduct:
    ...
    @pytest.mark.asyncio
    async def test_raises_when_agent_unavailable(self) -> None:
        """Raises RuntimeError when agent is unavailable (fail-fast)."""
        mock_session = MagicMock(spec=Session)
        product = Product(
            id="prod_1",
        ...
        with pytest.raises(RuntimeError, match="Promotion agent returned no decision"):
            await get_promotion_for_product(mock_session, product, mock_client)

    @pytest.mark.asyncio
    async def test_raises_if_discount_violates_margin(self) -> None:
        """Raises ValueError if agent decision violates margin constraint."""
        mock_session = MagicMock(spec=Session)
        product = Product(
            id="prod_1",
            sku="TEST-001",
        ...
        with pytest.raises(ValueError, match="violates margin for"):
            await get_promotion_for_product(mock_session, product, mock_client)
...
class TestGetPromotionsForProducts:
    ...
    async def test_returns_promotions_for_all_products(self) -> None:
        ...
        assert all(r["action"] == "DISCOUNT_10_PCT" for r in results)

    @pytest.mark.asyncio
    async def test_handles_exceptions_gracefully(self) -> None:
        """Fails fast if any individual product promotion fails."""
        mock_session = MagicMock(spec=Session)
        products = [
            Product(
                id="prod_1",
