...
def get_engine():
    ...
    if _engine is None:
        ...
        # Industrial Scale: Enhanced Connection Pooling for Global Traffic
        if not is_sqlite:
    ...
    for p in PRODUCTS:
        session.add(
            Product(
                ...
                tagline=p.get(
                    "name", ""
                ),  # Using name as tagline for variety if missing
                features_json=json.dumps(p.get("attributes", [])),
...
def init_and_seed_db() -> None:
    ...
    Industrial Fail-Open: If the DB is unavailable during startup,
