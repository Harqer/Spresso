...
async def firebase_webhook(
    ...
    try:
        ...
        with next(get_session()) as session:
            ...
            if not existing:
                new_customer = Customer(id=user_id, email=email, name=name, tier="free")
...
async def stripe_webhook(request: Request):
    ...
    if not sig_header or not webhook_secret:
        logger.error(
            "Security Alert: Stripe webhook triggered without signature or secret."
        )
        ...
        if order_id:
            with next(get_session()) as session:
                ...
                if order:
                    ...
                    session.commit()
                    logger.info(
                        f"Order Synchronized: {order_id} marked as COMPLETED via Webhook."
                    )
...
async def github_webhook(request: Request):
    ...
    if not webhook_secret:
        logger.warning(
            "No GITHUB_WEBHOOK_SECRET configured, skipping signature validation."
        )
    elif not sig_header:
    ...
    if event_type == "dependabot_alert":
        ...
        summary = alert.get("security_advisory", {}).get(
