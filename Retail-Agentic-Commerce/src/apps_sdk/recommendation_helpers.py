...
async def call_recommendation_agent(
    ...
    """Call the ARAG recommendation agent."""
    if settings.use_gemini:
        # Import here to avoid circular dependency or unnecessary load if not using Gemini
        ...
        try:
            parsed_result = await gemini.get_recommendations(
                agent_cart_items, session_context
            )
