...
class ProductRepository {
    ...
    suspend fun getRecommendations(userToken: String, query: String = "featured"): List<Product> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun createCheckoutSession(items: List<Product>, userToken: String): String? = withContext(Dispatchers.IO) {
        try {
            ...
            val requestBody = JSONObject().apply {
                ...
                put("buyer", JSONObject().apply {
                    put("first_name", "Vaultier")
                    put("last_name", "User")
                    put("email", "user@vaultier.ai")
                })
                put("fulfillment_address", JSONObject().apply {
                    put("name", "Default Address")
                    put("line_one", "123 Agentic Way")
                    put("city", "San Francisco")
                    put("state", "CA")
                    put("country", "US")
                    put("postal_code", "94105")
                })
            }
    ...
    suspend fun completeCheckout(sessionId: String, paymentToken: String, userToken: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            val requestBody = JSONObject().apply {
                put("payment_data", JSONObject().apply {
